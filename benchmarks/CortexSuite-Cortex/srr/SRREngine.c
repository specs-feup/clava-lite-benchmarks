...
void get_g(int rn, int pn)
{
    ...
    else
    {
        for (i = 0; i < l * l; i++)
        {
            ...
            if (mv[i].x > (double)1.0)
                temp_pn = pn - floor(mv[i].x);
            else if (mv[i].x < (double)-1.0)
                temp_pn = pn + abs(floor(mv[i].x)) - 1;
            else
                temp_pn = pn;
            // if(abs(mv[i].y) > (double)1.0) rn -= floor(mv[i].y);
            if (mv[i].y > (double)1.0)
                temp_rn = rn - floor(mv[i].x);
            else if (mv[i].y < (double)-1.0)
                temp_rn = rn + abs(floor(mv[i].x)) - 1;
            else
                temp_rn = rn;
